...
class Subpage_lists extends CI_Controller
{
	...
	public function list($id)
	{
		if (checkAccess()) {
			...
			$data['rows'] = $this->back_m->get_all_where($this->uri->segment(2), 'subpage_id', $id);
			$data['subpage'] = $this->back_m->get_one('subpages', $id);
	...
	public function form($type, $subpage_id, $id = '')
	{
		if (checkAccess()) {
			// DEFAULT DATA
			$data = loadDefaultData();
			...
			$data['subpage'] = $this->back_m->get_one('subpages', $subpage_id);
	...
	public function priority()
	{
		...
		$this->back_m->update('subpage_lists', $insert, $_POST['id']);
	}

	public function action($type, $table, $subpage_id, $id = '')
	{
		if (checkAccess()) {
			$now = date('Y-m-d');
			...
			redirect("panel/$table/list/$subpage_id");
		} else {
			redirect('panel');
		}
	}
