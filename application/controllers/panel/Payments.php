...
class Payments extends CI_Controller
{
		...
		if (checkAccess()) {
			...
			foreach ($_POST as $key => $value) {
				...
				if (!$this->db->field_exists('photo', $table)) {
					$this->base_m->create_column($table, 'photo');
				}
				if (!$this->db->field_exists('photo_webp', $table)) {
					$this->base_m->create_column($table, 'photo_webp');
				}
	...
	public function active_detal()
	{
		$insert['active_detal'] = $_POST['value'];
		$this->back_m->update($_POST['table'], $insert, $_POST['id']);
	}

	public function active_hurt()
	{
		$insert['active_hurt'] = $_POST['value'];
		$this->back_m->update($_POST['table'], $insert, $_POST['id']);
	}
}
